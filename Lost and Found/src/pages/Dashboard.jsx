...
const confirmDelivered = async (matchId) => {
  ...
  try {
    const res = await fetch(
      'http://localhost/lostfound/update_match_status.php',
      {
        method: 'POST',
...
const hideMatch = async (matchId) => {
  ...
  try {
    const res = await fetch(
      `http://localhost/lostfound/hide_match.php?match_id=${matchId}&user_id=${user.id}`
    )
                ...
                <Card key={item.id} className="border border-white/20 bg-white/10 backdrop-blur-xl shadow-xl text-white">
                  <CardContent className="py-4">
                    <div className="flex gap-4">
                      {item.image_url && (
  <img
    src={`http://localhost/lostfound/${item.image_url}`} // prepend backend URL
    alt={item.title}
